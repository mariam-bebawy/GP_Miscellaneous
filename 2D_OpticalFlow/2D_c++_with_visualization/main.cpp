...
int main(int argc, const char** argv)
{
    ...
    start = clock();
    calculate_opticalflow(&flowfields,cap,frame_Number,Mask);
    end = clock();
    ...
    cout << "dimensions = " << flowfields[0].rows << " , " << flowfields[0].cols;
    start = clock();
    vector<vector<Mat>> integratedflowfields;
    integratedflowfields=integrateflowfield(&flowfields, Mask_nuc);
