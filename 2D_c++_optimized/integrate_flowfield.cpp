...
void integrateflowfield(GpuMat* flowfields, GpuMat*xp, GpuMat*yp,int flowfields_len, GpuMat Mask){
	...
	xp[0].upload(x);
	yp[0].upload(y);
	//cv::cuda::multiply(xp[0], Mask, xp[0]);
	//cv::cuda::multiply(yp[0], Mask, yp[0]);

	//cv::cuda::bitwise_and(xp[0], Mask, xp[0]);
	//cv::cuda::bitwise_and(yp[0], Mask, yp[0]);
	...
	for (int t = 0; t < flowfields_len; t++) {
		...
		//cv::cuda::bitwise_and(xp[t+1], Mask, xp[t + 1]);
		//cv::cuda::bitwise_and(yp[t + 1], Mask, yp[t + 1]);
